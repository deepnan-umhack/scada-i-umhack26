...
// netlify/functions/update-bookings.ts
var update_bookings_exports = {};
__export(update_bookings_exports, {
  handler: () => handler
});
module.exports = __toCommonJS(update_bookings_exports);
...
var handler = schedule("*/5 * * * *", async (event) => {
  ...
  try {
    const { error: progressError } = await supabase.from("bookings").update({ status: "IN PROGRESS" }).lte("start_time", now).gt("end_time", now).in("status", ["PENDING", "CONFIRMED"]);
    if (progressError) throw progressError;
    const { error: completedError } = await supabase.from("bookings").update({ status: "COMPLETED" }).lte("end_time", now).neq("status", "COMPLETED").neq("status", "CANCELLED");
...
//# sourceMappingURL=update-bookings.js.map
