from core.schemas import UserRequest, OptimizerRequest
...
from utils.demo_logger import title, step, final_block
...
def handle_hvac_request(user_request: UserRequest) -> dict:
    ...
    booking = get_booking_context(actual_room_id)

    decision = hvac_controller(room_state, normalized_request, policy, booking)
    ...
    if decision.action == "SET_HVAC":
        ...
        pause_result = pause_optimizer(
            room_id=decision.room_id,
            duration_min=decision.duration_min or 60
        )
        result["optimizer_pause"] = pause_result
        step("⏸️", f"Optimizer paused for {decision.duration_min or 60} minutes")

    elif decision.action == "OPTIMIZE_AC":
        if is_optimizer_paused(decision.room_id):
            result["execution"] = {
                "status": "blocked",
                "reason": "Optimizer is currently paused due to active manual override."
            }
            result["optimizer_pause"] = get_optimizer_pause_status(decision.room_id)
            step("⛔", "Optimizer is currently paused")
