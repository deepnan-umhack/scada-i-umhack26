...
def hvac_controller(
    ...
    # =========================
    # 3. PRE-COOLING TRIGGER
    # Rule-based decision for WHEN to start
    # Optimizer will decide HOW to cool
    # =========================
    elif (
        booking
        and booking.minutes_until_next_booking is not None
        and 0 <= booking.minutes_until_next_booking <= policy.pre_cooling_window_min
        and room_state.temperature_c > policy.max_temperature_c
    ):
        action = "OPTIMIZE_AC"
        duration = booking.minutes_until_next_booking
        reason = (
            f"Pre-cooling triggered. Next booking starts in "
            f"{booking.minutes_until_next_booking} minutes."
        )
        optimizer_used = True
